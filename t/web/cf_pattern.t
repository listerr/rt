...
for my $page ("/Ticket/Create.html?Queue=1", "/Ticket/Modify.html?id=".$ticket->id) {
    ...
    $m->content_contains("Input must match Buffalo");
    $m->content_contains("Zebus");
    $m->content_contains("Input must match AKA Cebu");
...
diag "Quick ticket creation";
{
    ...
    $m->content_contains("Bison: Input must match", "Found CF validation error Bison");
    $m->content_contains("Zebus: Input must match", "Found CF validation error Zebus");
